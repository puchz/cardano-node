# Note: Check libsodium checkout version and final copy, node and cli version.
...
RUN wget --no-check-certificate https://hydra.iohk.io/build/${NODE_BUILD_NUM}/download/1/mainnet-alonzo-genesis.json
...
RUN wget --no-check-certificate https://hydra.iohk.io/build/${NODE_BUILD_NUM}/download/1/testnet-alonzo-genesis.json
...
COPY --from=builder /build/cardano-node/cardano-node/dist-newstyle/build/x86_64-linux/ghc-8.10.2/cardano-node-1.27.0/x/cardano-node/build/cardano-node/cardano-node /usr/local/bin/
COPY --from=builder /build/cardano-node/cardano-node/dist-newstyle/build/x86_64-linux/ghc-8.10.2/cardano-cli-1.27.0/x/cardano-cli/build/cardano-cli/cardano-cli /usr/local/bin/
