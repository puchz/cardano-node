...
ARG OS_VERSION=linux
WORKDIR /build/ghc
RUN wget https://downloads.haskell.org/~ghc/${GHC_VERSION}/ghc-${GHC_VERSION}-${OS_ARCH}-${OS_VERSION}.tar.xz
RUN tar -xf ghc-${GHC_VERSION}-${OS_ARCH}-${OS_VERSION}.tar.xz
